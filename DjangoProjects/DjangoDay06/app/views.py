...
def register(request):
    if request.method == 'GET':
        ...
        # 状态保持 session
        # 客户端存储 id:session_key   服务器存储session_key:session_data(username)
...
def login(request):
    if request.method == 'GET':
        ...
        # if not username :
        #     print('aaa')
        #     return render(request,'index.html',context={'err':'请输入用户名密码'})

        users = User.objects.filter(username=username).filter(password=password)
        if users.exists():
            # 状态保持(暂时不能不同客户端同时登陆同一个账户)
            user = users.first()
...
def upfile(request):
    if request.method == 'GET':
        ...
        with open(filepath, 'wb') as fp:
            for item in headimg.chunks():    # 分块读写
                fp.write(item)
